    ...
    for i in range(len(queries)):
        ...
        for j in range(len(data)):
            if np.abs(indices[i][0] - indices[data[j]][0]) > tau and np.abs(indices[i][1] - indices[data[j]][1]) > tau:
                dist = minkowski_distance_p(queries[i], queries[data[j]]) # 闵式距离
                # e_dist = e(queries[i], queries[data[j]])
                if dist< distances[i]:
                    distances[i] = dist
                    offsets[i] = [indices[data[j]][0] - indices[i][0], indices[data[j]][1] - indices[i][1]]
    return distances, offsets    

def e(edge_patch0, edge_patch1):
    if structural_similarity(edge_patch0, edge_patch1) >= 0.5:
        return 0
    # if np.linalg.norm(edge_patch0-edge_patch1) < 200:
    #     return 0
    return np.inf